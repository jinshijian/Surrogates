...
### Model statistics results for Tm surrogates using PCA

```{r Tm surrogate PCA}
t_results %>% select(p_b, first_R2, p_Tm_b, first_Tm_R2, first_b, first_Tm_b, p_TsTair_b, TsTair_R2, Biome_group) %>% 
  na.omit() -> sub_t_results

sub_t_results %>% mutate(group = case_when(p_b < 0.05 & p_Tm_b < 0.05 ~ "A",
                                           p_b < 0.05 & p_Tm_b >= 0.05 ~ "B",
                                           p_b >= 0.05 & p_Tm_b < 0.05 ~ "C",
                                           TRUE ~ "D")) -> sub_t_results

# rename columns
colnames(sub_t_results) <- c("p(Rs~Tsoil)", "R2(Rs~Tsoil)", "p(Rs~Tair)", "R2(Rs~Tair)",
                             "beta(Rs~Tsoil)", "beta(Rs~Tair)", "p(Tsoil~Tair)", "R2(Tsoil~Tair)", "Climate", "group" )

write.csv(sub_t_results, "outputs/sub_t_results.csv", col.names = FALSE)

prcomp(sub_t_results %>% select(-group, -Climate, -"beta(Rs~Tsoil)", -"beta(Rs~Tair)") %>% na.omit(),
             center = TRUE,
             scale. = TRUE) -> t_PCA
...
g_climate <- ggbiplot(t_PCA,
              obs.scale = 1,
              var.scale = 1,
              groups = sub_t_results$Climate,
              ellipse = T,
              circle = F,
              ellipse.prob = 0.9,
              varname.size = 0,
...
plot_grid(g_p, g_climate,
          labels = c("  a  ", "  b  "),
          label_size = 12,
          vjust = 10, hjust = c(-3.25),
          rel_widths = c(1, 1.25),
          ncol = 2 )

ggsave("outputs/Figure2.jpg", dpi = 300, width = 8, height = 5, units = "in")
...
# ggsave("outputs/FigureS6.jpg", width = 8, height = 5, dpi = 300, units = "in")
...
```{r Pm surrogate PCA}
swc_results %>% select(p_b, slr_d, p_pm_b, first_pm_b, first_b, slr_pm_d, p_SWCPm_b, SWCPm_b, Biome_group) %>% 
  na.omit() -> sub_swc_results

sub_swc_results %>% mutate(group = case_when(p_b < 0.05 & p_pm_b < 0.05 ~ "A",
                                           p_b < 0.05 & p_pm_b >= 0.05 ~ "B",
                                           p_b >= 0.05 & p_pm_b < 0.05 ~ "C",
                                           TRUE ~ "D")) -> sub_swc_results

# rename columns
colnames(sub_swc_results) <- c("p(Rs~SWC)", "R2(Rs~SWC)", "p(Rs~Pm)", "R2(Rs~Pm)", "beta(Rs~SWC)", "beta(Rs~Pm)"
                               , "p(SWC~Pm)", "R2(SWC~Pm)", "Climate", "group" )


prcomp(sub_swc_results %>% select(-group, -Climate
                                  # , -"beta(Rs~SWC)", -"beta(Rs~Pm)"
                                  ,-"R2(Rs~SWC)", -"R2(Rs~Pm)", -"R2(SWC~Pm)"
                                  ) %>% na.omit(),
             center = TRUE,
             scale. = TRUE) -> swc_PCA
...
g_swc <- ggbiplot(swc_PCA,
              obs.scale = 1,
              var.scale = 1,
              groups = sub_swc_results$group,
              ellipse = T,
              circle = F,
              ellipse.prob = 0.9,
              varname.size = 3,
...
g_climate_swc <- ggbiplot(swc_PCA,
              obs.scale = 1,
              var.scale = 1,
              groups = sub_swc_results$Climate,
              ellipse = T,
              circle = F,
              ellipse.prob = 0.9,
              varname.size = 0,
...
plot_grid(g_swc, g_climate_swc,
          labels = c("a", "b"),
          label_size = 12,
          vjust = 10.5, hjust = c(-8, -7.25),
          rel_widths = c(1, 1.07),
          ncol = 2 )

# ggsave("outputs/Figure4.jpg", dpi = 300, width = 8.25, height = 4, units = "in")
# ggbiplot(swc_PCA)
```
...
# ggsave("outputs/FigureS10.jpg", width = 8, height = 5, dpi = 300, units = "in")
...
# ggsave("outputs/FigureS11.jpg", dpi = 300, width = 8, height = 7, units = "in")
```
...
# ggsave("outputs/FigureS12.jpg", dpi = 300, width = 8, height = 7, units = "in")
```
...
# ggsave("outputs/FigureS3.jpg", dpi = 300, width = 8, height = 6, units = "in")
...
# ggsave("outputs/Figure6.jpg", width = 8, height = 7, dpi = 300, units = "in")
...
# plot_vwc
ggsave("outputs/FigureS11.jpg", width = 8, height = 7, dpi = 300, units = "in")
...
# ggsave("outputs/FigureS12.jpg", width = 4, height = 6, units = "in", dpi = 300)
...
t_results %>% filter(p_b <= 0.05) %>% nrow()
t_results %>% filter(p_Tm_b <= 0.05) %>% select (p_b, p_Tm_b, first_R2, first_Tm_R2) %>% summarise(median(first_Tm_R2))
t_results %>% filter(p_Tm_b <= 0.05) %>% nrow()

t_results %>% filter(p_c <= 0.05) %>% select (p_c, p_Tm_c, poly_R2, poly_Tm_R2) %>% summarise(median(poly_R2))
...
```{r Temperature threshold, fig.height=4, fig.width=4}
...
# ggsave("outputs/FigureS.jpg", width = 4, height = 4, dpi = 300, units = "in")
...
t_results %>% select(p_b, first_R2, p_Tm_b, first_Tm_R2, first_b, first_Tm_b, p_TsTair_b, TsTair_R2
                     , Biome_group, Ecosystem_group, Leaf_group, Drainage_group
                     , MAT, MAP, Ele
                     ) %>% 
  na.omit() -> sub_t_results

sub_t_results$MAT_factor <- factor(cut( sub_t_results$MAT, 6 ))
sub_t_results$MAP_factor <- factor(cut( sub_t_results$MAP, 6 ))
sub_t_results$Ele_factor <- factor(cut( sub_t_results$Ele, 6 ))
...
# rename columns
colnames(sub_t_results) <- c("p(Rs~Tsoil)", "R2(Rs~Tsoil)", "p(Rs~Tair)", "R2(Rs~Tair)",
                             "beta(Rs~Tsoil)", "beta(Rs~Tair)", "p(Tsoil~Tair)", "R2(Tsoil~Tair)", "Climate"
                             , "Ecosystem", "Leaf", "Drainage"
                             , "MAT", "MAP", "Ele", "MAT_factor", "MAP_factor", "Ele_factor"
                             , "group" )

sub_t_results %>% mutate(Ecosystem = case_when(Ecosystem == "Agriculture"~"Ag",
                                   Ecosystem == "Grassland"~"Gra",
                                   Ecosystem == "Savanna"~"Sav",
                                   Ecosystem == "Desert"~"Des",
                                   Ecosystem == "Shrubland"~"Shr",
                                   Ecosystem == "Forest"~"For",
                                   Ecosystem == "Plantation"~"Pla",
                                   Ecosystem == "Wetland"~"Wet",
                                   TRUE~"NR")) -> sub_t_results


sub_t_results %>% mutate(Drainage = case_when(Drainage == "Dry"~"Dry ",
                                          Drainage == "Wet"~"Wet ",
                                          TRUE~"NR")) -> sub_t_results

# unique(sub_t_results$MAP_factor)
sub_t_results %>% mutate(MAP_factor = case_when(MAP_factor == "(188,936]"~" (188,936]",
                                                MAP_factor == "(936,1.68e+03]"~"(936,1680]",
                                                MAP_factor == "(1.68e+03,2.42e+03]"~"(1680,2420]",
                                                MAP_factor == "(2.42e+03,3.17e+03]"~"(2420,3170]",
                                                MAP_factor == "(3.17e+03,3.91e+03]"~"(3170,3910]",
                                                MAP_factor == "(3.91e+03,4.66e+03]"~"(3910,4660]",
                                                TRUE~"NR")) -> sub_t_results

# unique(sub_t_results$Ele_factor)
                  
sub_t_results %>% mutate(Ele_factor = case_when(Ele_factor == "(-3.13,774]"~" (-3.13,774]",
                                                Ele_factor == "(774,1.55e+03]"~" (774,1550]",
                                                Ele_factor == "(1.55e+03,2.32e+03]"~"(1550,2320]",
                                                Ele_factor == "(2.32e+03,3.09e+03]"~"(2320,3090]",
                                                Ele_factor == "(3.09e+03,3.86e+03]"~"(3090,3860]",
                                                Ele_factor == "(3.86e+03,4.64e+03]"~"(3860,4640]",
                                                TRUE~"NR")) -> sub_t_results

prcomp(sub_t_results %>% select(-group, -Climate, -MAT, -MAP, -Ele, 
                                -"Ecosystem", -"Leaf", -"Drainage",
                                - "MAT_factor", - "MAP_factor", - "Ele_factor",
                                -"beta(Rs~Tsoil)", -"beta(Rs~Tair)") %>% na.omit(),
             center = TRUE,
             scale. = TRUE) -> t_PCA
...
plot_grid(pca_plot(sub_t_results$Ecosystem),
          pca_plot(sub_t_results$Leaf),
          pca_plot(sub_t_results$Drainage),
          pca_plot(sub_t_results$MAT_factor),
          pca_plot(sub_t_results$MAP_factor),
          pca_plot(sub_t_results$Ele_factor),
          labels = c("Ecosystem", "Leaf", "Drainage", "MAT", "MAP", "Elevation"),
          label_size = 9,
          vjust = 11, hjust = c(-1.3,-3.5,-1.725,-3.25,-3.25,-1.6),
          ncol = 3 )

# ?plot_grid()
# ggsave("outputs/FigureS5.jpg", dpi = 300, width = 9, height = 7, units = "in")

```


```{r}
# hierarchy classfication
sub_t_results %>% 
  select("Climate", "Ecosystem", "Leaf", "Drainage", "MAT", "MAP", "Ele") %>% 
  na.omit() ->
  sub_t_cluster

hclust_avg <- hclust(sub_t_cluster, method = 'average')
plot(hclust_avg)
```
...
```{r Figure-S13 Temperature-Resicual}
...
ggsave("outputs/FigureS13.jpg", dpi = 300, width = 8, height = 8, units = "in")
```
...
ggsave("outputs/FigureS8.jpg", width = 8, height = 6, dpi = 300, units = "in")
...
swc_results %>% filter(p_b < 0.05 & p_pm_b < 0.05) %>% nrow()
swc_results %>% filter(p_b < 0.05 & p_pm_b < 0.05) %>% select (p_b, p_pm_b, first_R2, first_pm_R2) %>% summarise(median(first_R2))
swc_results %>% filter(p_pm_b < 0.05 & p_pm_b < 0.05) %>% select (p_b, p_pm_b, first_R2, first_pm_R2) %>% summarise(median(first_pm_R2))

```



```{r SWC threshold, fig.height=6, fig.width=4}
# threshold of GWC
...
# ggsave("outputs/FigureS.jpg", width = 4, height = 6, dpi = 300, units = "in")
...
swc_results %>% select(p_b, first_R2, p_pm_b, first_pm_R2, first_b, first_pm_b, p_SWCPm_b, SWCPm_R2
                     , Biome_group, Ecosystem_group, Leaf_group, Drainage_group
                     , MAT, MAP, Ele
                     , Clay, BD, OC
                     ) %>% 
...
sub_swc_results$MAT_factor <- factor(cut( sub_swc_results$MAT, 6 ))
sub_swc_results$MAP_factor <- factor(cut( sub_swc_results$MAP, 6 ))
sub_swc_results$Ele_factor <- factor(cut( sub_swc_results$Ele, 6 ))

sub_swc_results$Clay_factor <- factor(cut( sub_swc_results$Clay, 6 ))
sub_swc_results$BD_factor <- factor(cut( sub_swc_results$BD, 6 ))
sub_swc_results$OC_factor <- factor(cut( sub_swc_results$OC, 6 ))
...
# rename columns
colnames(sub_swc_results) <- c("p(Rs~SWC)", "R2(Rs~SWC)", "p(Rs~Pm)", "R2(Rs~Pm)"
                               ,"beta(Rs~SWC)", "beta(Rs~Pm)", "p(SWC~Pm)", "R2(SWC~Pm)", "Climate"
                               , "Ecosystem", "Leaf", "Drainage"
                               , "MAT", "MAP", "Ele"
                               , "Clay", "BD", "OC"
                               , "MAT_factor", "MAP_factor", "Ele_factor"
                               , "Clay_factor", "BD_factor", "OC_factor"
                               , "group" )

sub_swc_results %>% mutate(Ecosystem = case_when(Ecosystem == "Agriculture"~"Ag",
                                   Ecosystem == "Grassland"~"Gra",
                                   Ecosystem == "Savanna"~"Sav",
                                   Ecosystem == "Desert"~"Des",
                                   Ecosystem == "Shrubland"~"Shr",
                                   ...
                                   Ecosystem == "Plantation"~"Pla",
                                   Ecosystem == "Wetland"~"Wet",
                                   TRUE~"NR")) -> sub_swc_results


sub_swc_results %>% mutate(Drainage = case_when(Drainage == "Dry"~"Dry ",
                                          Drainage == "Wet"~"Wet ",
                                          TRUE~"NR")) -> sub_swc_results

# unique(sub_swc_results$MAP_factor)
sub_swc_results %>% mutate(MAP_factor = case_when(MAP_factor == "(188,936]"~" (188,936]",
                                                MAP_factor == "(936,1.68e+03]"~" (936,1680]",
                                                MAP_factor == "(1.68e+03,2.42e+03]"~"(1680,2420]",
                                                MAP_factor == "(2.42e+03,3.17e+03]"~"(2420,3170]",
                                                MAP_factor == "(3.17e+03,3.91e+03]"~"(3170,3910]",
...
# unique(sub_swc_results$Ele_factor)
                  
sub_swc_results %>% mutate(Ele_factor = case_when(Ele_factor == "(0.37,777]"~" (0.37,777]",
                                                Ele_factor == "(777,1.55e+03]"~" (777,1550]",
                                                Ele_factor == "(1.55e+03,2.32e+03]"~"(1550,2320]",
                                                Ele_factor == "(2.32e+03,3.09e+03]"~"(2320,3090]",
                                                # Ele_factor == "(3.09e+03,3.86e+03]"~"(3090,3860]",
                                                Ele_factor == "(3.86e+03,4.64e+03]"~"(3090,4640]",
                                                TRUE~"NR")) -> sub_swc_results


# Get clay%, OC%, and BD from HWSD *********** ********** **********
# get soil CEC



prcomp(sub_swc_results %>% select(-group, -Climate, -MAT, -MAP, -Ele, -Clay, -BD, -OC,
                                -"Ecosystem", -"Leaf", -"Drainage",
                                - "MAT_factor", - "MAP_factor", - "Ele_factor",
                                -"Clay_factor", -"BD_factor", -"OC_factor",
                                -"beta(Rs~SWC)", -"beta(Rs~Pm)") %>% na.omit(),
             center = TRUE,
             scale. = TRUE) -> swc_PCA
...
pca_plot_swc <- function(var_factor) {
  g1 <- ggbiplot(swc_PCA,
              obs.scale = 1,
              var.scale = 1,
              groups = var_factor,
...
plot_grid(pca_plot_swc(sub_swc_results$Ecosystem),
          pca_plot_swc(sub_swc_results$Leaf),
          pca_plot_swc(sub_swc_results$Drainage),
          pca_plot_swc(sub_swc_results$MAT_factor),
          pca_plot_swc(sub_swc_results$MAP_factor),
          pca_plot_swc(sub_swc_results$Ele_factor),
          pca_plot_swc(sub_swc_results$Clay_factor),
          pca_plot_swc(sub_swc_results$BD_factor),
          pca_plot_swc(sub_swc_results$OC_factor),
          labels = c("Ecosystem", "Leaf", "Drainage", "MAT", "MAP", "Elevation", "Clay%", "BD", "OC"),
          label_size = 9,
          vjust = 11.5, hjust = c(-1.3,-3.5,-1.725,-3.25,-3.25,-1.6, -2.15,-4.75,-4.75),
          ncol = 3 )

# ggsave("outputs/FigureS9.jpg", dpi = 300, width = 9, height = 10, units = "in")
...
```{r Figure-S14 SWC-Resicual plot}
# reorganize data for plot
bind_rows(
  # SLR-SWC
...
ggsave("outputs/FigureS14.jpg", dpi = 300, width = 8, height = 8, units = "in")
...
```{r}
DGRsD %>% select(SWC_Type) %>% unique()
```

```{r, fig.width=8, fig.height=5}
# linear regression between prediction from Tsoil and Tair
plot_grid(
  t_residual_results %>% 
    ggplot(aes(slr_tm_pred, slr_pred)) +
    geom_point(alpha = 0.5) +
    geom_abline(slope = 1) +
    geom_smooth(method = "lm") +
    labs(y = expression(Predicted~R[S]~by~Tsoil),
         x = expression(Predicted~R[S]~by~Tair)),
  
  swc_residual_results %>% 
    ggplot(aes(slr_pm_pred, slr_pred)) +
    geom_point(alpha = 0.5) +
    geom_abline(slope = 1) +
    geom_smooth(method = "lm") +
    labs(y = expression(Predicted~R[S]~by~SWC),
         x = expression(Predicted~R[S]~by~Pm))
)
...
sub_swc_cluster <- sub_swc_results[,c(1,3)] 
sub_swc_cluster <- scale(sub_swc_cluster)
head(sub_swc_cluster)
k4 <- kmeans(sub_swc_cluster, centers = 4, nstart = 25)
fviz_cluster(k4, data = sub_swc_cluster)

sub_swc_results %>%
  mutate(cluster = k4$cluster) %>%
  ggplot(aes(sub_swc_results$`p(Rs~SWC)`, sub_swc_results$`p(Rs~Pm)`, color = factor(cluster), label = group)) +
  geom_text()
