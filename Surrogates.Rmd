...
ggplot(aes(Tm_Del, Pm_Del), data = DGRsD) + geom_bin2d(alpha = 0.5) + 
  ...
  geom_point(data = DGRsD_TS, aes(Tm_Del, Pm_Del, col = Climate_type), alpha = 0.15, legend = FALSE) +
...
```{r Pm surrogate PCA}
swc_results %>% select(p_b, first_R2, p_pm_b, first_pm_R2, first_b, first_pm_b, p_SWCPm_b, SWCPm_R2, Biome_group) %>% 
...
prcomp(sub_swc_results %>% select(-group, -Climate, -"beta(Rs~SWC)", -"beta(Rs~Pm)") %>% na.omit(),
             center = TRUE,
             scale. = TRUE) -> swc_PCA
