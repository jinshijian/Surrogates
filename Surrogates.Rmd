...
DGRsD_TS %>% filter(!is.infinite(log_rs) & !is.na(log_rs)) -> DGRsD_TS
DGRsD_SWC = rm_dgrsd_swc_null(DGRsD)
DGRsD_SWC %>% filter(!is.infinite(log_rs)& !is.na(log_rs)) -> DGRsD_SWC
...
```{r Figure 1, fig.height=8, fig.width=10}
...
ggsave("outputs/Figure1. Site_distribution.jpg", width = 8, height = 6, dpi = 300, units = "in")
```
...
p4 = p4 + coord_polar("y", start=0) + geom_text(aes(label = paste0(round(freq, 2), "%")), position = position_stack(vjust = 0.5)) +
  scale_fill_manual(values=c("violet", "orange", "gray")) +
  labs(x = NULL, y = NULL, fill = NULL, title = expression("(d) NLR"~R^{2}~"difference")) +
  theme_classic() + theme(axis.line = element_blank(),
          axis.text = element_blank(),
          axis.ticks = element_blank(),
          legend.position = "bottom",
...
p5 = p5 + coord_polar("y", start=0) + geom_text(aes(label = paste0(round(freq, 2), "%")), position = position_stack(vjust = 0.5)) +
  scale_fill_manual(values=c("pink", "cyan")) +
  labs(x = NULL, y = NULL, fill = NULL, title = expression("(e) NLR(R"[S]~"-vs-T"[soil]~")")) +
  theme_classic() + theme(axis.line = element_blank(),
          axis.text = element_blank(),
          axis.ticks = element_blank(),
          legend.position = "bottom",
...
p6 = p6 + coord_polar("y", start=0) + geom_text(aes(label = paste0(round(freq, 2), "%")), position = position_stack(vjust = 0.5)) +
  scale_fill_manual(values=c("pink", "cyan")) +
  labs(x = NULL, y = NULL, fill = NULL, title = expression("(f) NLR(R"[S]~"-vs-T"[air]~")")) +
  theme_classic() + theme(axis.line = element_blank(),
          axis.text = element_blank(),
          axis.ticks = element_blank(),
          legend.position = "bottom",
...
bind_rows(
  ...
  t_results %>% select(`Tsoil` = poly_b, `Tair` = poly_Tm_b, p_poly_b, p_poly_Tm_b, p_c, p_Tm_c) %>% 
    ...
    mutate(Model = 'NLR', Model_stat = 'Slope')
) -> t_surrogate_beta
...
bind_rows(
  ...
  t_results %>% select(`Tsoil` = poly_R2, `Tair` = poly_Tm_R2, p_poly_b, p_poly_Tm_b, p_c, p_Tm_c) %>% 
    filter(p_poly_b < 0.05 | p_c < 0.05 & p_poly_Tm_b < 0.05 | p_Tm_c < 0) %>% 
    mutate(Model = 'NLR', Model_stat = 'R2')
) -> t_surrogate_r2
...
bind_rows(
  ...
  t_results %>% select(`Tsoil` = poly_RMSE, `Tair` = poly_Tm_RMSE, p_poly_b, p_poly_Tm_b, p_c, p_Tm_c) %>%
    filter(p_poly_b < 0.05 | p_c < 0.05 & p_poly_Tm_b < 0.05 | p_Tm_c < 0) %>%
    filter(Tsoil < 2) %>%
    mutate(Model = 'NLR', Model_stat = 'RMSE')
) -> t_surrogate_rmse
...
bind_rows(
  ...
  t_results %>% select(`Tsoil` = poly_EF, `Tair` = poly_Tm_EF, p_poly_b, p_poly_Tm_b, p_c, p_Tm_c) %>% 
    filter(p_poly_b < 0.05 | p_c < 0.05 & p_poly_Tm_b < 0.05 | p_Tm_c < 0) %>% 
    mutate(Model = 'NLR', Model_stat = 'EF')
) -> t_surrogate_ef
...
# only XXX site with significant polynomial trend
...
p5 = p5 + coord_polar("y", start=0) + geom_text(aes(label = paste0(round(freq, 2), "%")), position = position_stack(vjust = 0.5)) +
  scale_fill_manual(values=c("pink", "cyan")) +
  labs(x = NULL, y = NULL, fill = NULL, title = expression("(e) NLR(R"[S]~"-vs-"~SWC~")")) +
  theme_classic() + theme(axis.line = element_blank(),
          axis.text = element_blank(),
          axis.ticks = element_blank(),
          legend.position = "bottom",
...
p6 = p6 + coord_polar("y", start=0) + geom_text(aes(label = paste0(round(freq, 2), "%")), position = position_stack(vjust = 0.5)) +
  scale_fill_manual(values=c("pink", "cyan")) +
  labs(x = NULL, y = NULL, fill = NULL, title = expression("(f) NLR(R"[S]~"-vs-"~P[m]~")")) +
  theme_classic() + theme(axis.line = element_blank(),
          axis.text = element_blank(),
          axis.ticks = element_blank(),
          legend.position = "bottom",
