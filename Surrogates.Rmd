...
# dist = 0.005
# t_surrogate_beta$outlier <- ifelse (t_surrogate_beta$cooks_dist > 0.1 | t_surrogate_beta$standardized_resids > 2, 'Yes', 'No')
...
bind_rows(
  ...
  t_surrogate_beta %>%
    filter(outlier == "No") %>%
    filter(Tsoil < 1.5) %>%
    select(Tsoil, Tair, p_poly_b, p_poly_Tm_b, p_c, p_Tm_c, Model, Model_stat), # remove two studies (4608 and 4609), these two studies have significant opposite trend),
  ...
  ggplot(aes(x = Tair, y = Tsoil )) + geom_point(alpha = 0.1) +
  geom_smooth(method = "lm", col = "blue", fill = "lightblue") +
  geom_abline(intercept = 0, slope = 1, col = "red", linetype = "dotdash", size = 1.25) +
  facet_wrap (Model ~ Model_stat, nrow = 2, scales = "free") +
  # scale_x_continuous(trans = "log2") +
