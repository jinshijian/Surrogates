...
bind_rows(
  ...
  t_results %>% select(`Tsoil` = poly_R2, `Tair` = poly_Tm_R2, p_poly_b, p_poly_Tm_b) %>% 
    filter(p_poly_b < 0.05 & p_poly_Tm_b < 0.05) %>% 
  ...
  t_results %>% select(`Tsoil` = poly_RMSE, `Tair` = poly_Tm_RMSE, p_poly_b, p_poly_Tm_b) %>% 
    filter(p_poly_b < 0.05 & p_poly_Tm_b < 0.05) %>% 
  ...
  t_results %>% select(`Tsoil` = poly_d, `Tair` = poly_Tm_d, p_poly_b, p_poly_Tm_b) %>% 
    filter(p_poly_b < 0.05 & p_poly_Tm_b < 0.05) %>% 
  ...
  t_results %>% select(`Tsoil` = poly_EF, `Tair` = poly_Tm_EF, p_poly_b, p_poly_Tm_b) %>% 
    filter(p_poly_b < 0.05 & p_poly_Tm_b < 0.05) %>% 
    mutate(Model = 'Polynomial', Model_stat = 'EF')
  
) %>% 
  ggplot(aes(x = Tair, y = Tsoil )) + geom_point(alpha = 0.25) +
  geom_smooth(method = "lm", col = "blue", fill = "lightblue") +
  geom_abline(intercept = 0, slope = 1, col = "red", linetype = "dashed", size = 2) +
  facet_wrap (Model ~ Model_stat, nrow = 2, scales = "free")

ggsave("outputs/Figure3.jpg", width = 8, height = 6, dpi = 300, units = "in")
...
# only 16 site with significant polynomial trend
swc_results %>% select(`SWC` = poly_d, `Pm` = poly_pm_d, p_poly_b, p_poly_pm_b) %>% 
  filter(p_poly_b < 0.05 & p_poly_pm_b < 0.05) %>% nrow()
...
bind_rows(
  ...
  swc_results %>% select(`SWC` = poly_R2, `Pm` = poly_pm_R2, p_poly_b, p_poly_pm_b) %>% 
    filter(p_poly_b < 0.05 & p_poly_pm_b < 0.05) %>% 
    mutate(Model = 'Polynomial', Model_stat = 'R2'), 
  ...
  swc_results %>% select(`SWC` = poly_RMSE, `Pm` = poly_pm_RMSE, p_poly_b, p_poly_pm_b) %>% 
    filter(p_poly_b < 0.05 & p_poly_pm_b < 0.05) %>% 
    mutate(Model = 'Polynomial', Model_stat = 'RMSE'),
  ...
  swc_results %>% select(`SWC` = poly_EF, `Pm` = poly_pm_EF, p_poly_b, p_poly_pm_b) %>% 
    filter(p_poly_b < 0.05 & p_poly_pm_b < 0.05) %>% 
    mutate(Model = 'Polynomial', Model_stat = 'EF')
  
) %>% 
  ggplot(aes(x = Pm, y = SWC )) + geom_point(alpha = 0.25) +
  geom_smooth(method = "lm", col = "blue", fill = "lightblue") +
  geom_abline(intercept = 0, slope = 1, col = "red", linetype = "dashed", size = 2) +
  facet_wrap (Model ~ Model_stat, nrow = 2, scales = "free")

ggsave("outputs/Figure5.jpg", width = 8, height = 6, dpi = 300, units = "in")
